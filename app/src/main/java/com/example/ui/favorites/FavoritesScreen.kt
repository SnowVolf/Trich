        ...
        Box(
            ...
            if (state.isLoading) {
                ...
                Text(
                    ...
                    modifier = Modifier.align(Alignment.Center).padding(top = padding.calculateTopPadding())
                )
            } else {
                LazyColumn(
...
@OptIn(ExperimentalFoundationApi::class)
@Composable
fun FavoriteItem(thread: FavoriteThread, isSelected: Boolean, onClick: () -> Unit, onLongClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .combinedClickable(
        ...
        border = androidx.compose.foundation.BorderStroke(1.dp, if (isSelected) MaterialTheme.colorScheme.primary else androidx.compose.ui.graphics.Color(0x0AFFFFFF))
    ) {
